...
ball = pygame.transform.scale(pygame.image.load('nlo.png').convert_alpha(), (80, 60))
ball_rect = ball.get_rect()
ball_speed = 5


def create_enemy():
    enemy = pygame.transform.scale(pygame.image.load('enemy.png').convert_alpha(), (70, 60))
    enemy_rect = pygame.Rect(width, random.randint(0, height), *enemy.get_size())
    enemy_speed = random.randint(0, 5)
    return [enemy, enemy_rect, enemy_speed]


def create_bonus():
    bonus = pygame.transform.scale(pygame.image.load('bonus.png').convert_alpha(), (70, 50))
    bonus_rect = pygame.Rect(random.randint(0, width), 0, *bonus.get_size())
    bonus_speed = random.randint(4, 6)
    return [bonus, bonus_rect, bonus_speed]


bg = pygame.transform.scale(pygame.image.load('back.png').convert(), screen)
bgX = 0
bgX2 = bg.get_width()
bg_speed = 3
